...
export default function PhaseIO() {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      fetchPhaseIOData({}, currentPage, pageSize);
  ...
  const handlePageSizeChange = (newPageSize: string) => {
    ...
    setPageSize(size);
    setCurrentPage(1);
    fetchPhaseIOData({}, 1, size);
  };

  const handlePageChange = (newPage: number) => {
    setCurrentPage(newPage);
    fetchPhaseIOData({}, newPage, pageSize);
  };
  ...
  const handleSearch = (searchParams: Record<string, string>) => {
    ...
    const timeout = setTimeout(() => {
      setCurrentPage(1);
      fetchPhaseIOData(searchParams, 1, pageSize);
    }, 500); // 0.5 seconds
